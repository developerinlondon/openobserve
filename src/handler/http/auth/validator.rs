...
async fn oo_validator_internal(
    ...
    // Trusted proxy authentication (oauth2-proxy, authentik, etc.)
    if cfg.auth.trusted_proxy_auth_enabled {
        if let Ok(header_name) = HeaderName::try_from(cfg.auth.trusted_proxy_user_header.as_str()) {
            if let Some(user_email) = req_data.headers.get(&header_name) {
                if let Ok(email) = user_email.to_str() {
                    if !email.is_empty() {
                        let role = match cfg.auth.trusted_proxy_default_role.to_lowercase().as_str() {
                            "admin" => UserRole::Admin,
                            "user" => UserRole::User,
                            "viewer" => UserRole::Viewer,
                            _ => UserRole::Admin,
                        };
                        log::debug!("Trusted proxy auth: {} with role {:?}", email, role);
                        return Ok(AuthValidationResult {
